  ...
  useFrame(({ camera }) => {
    ...
    if (!planeFound && alvaAR) {
      ...
      if (planePose) {
        ...
        let facingWeight = effectiveDot > FACING_THRESHOLD ? (effectiveDot - FACING_THRESHOLD) / (1 - FACING_THRESHOLD) : 0;

        // 수직성: 평면 노말과 월드 업 벡터의 내적 절대값
    ...
    // 오브젝트 배치: 평면 확정 후, 초기 후보와 현재 후보의 오프셋을 보정하여 배치

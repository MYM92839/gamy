...
import { ArToolkitSource, ArToolkitContext, ArMarkerControls } from './libs/threex.js';
    ...
    markerControls.addEventListener('markerFound', () => {
      ...
      if (userCoordRef.current) {
        correctedCoordRef.current = {
          lat: userCoordRef.current.lat + (markerCoord.lat - userCoordRef.current.lat),
          lon: userCoordRef.current.lon + (markerCoord.lon - userCoordRef.current.lon),
        };
        setState('stabilized');
      }
    });
